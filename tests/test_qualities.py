...
from app.engine import FORMATS_BY_KEY, Quality, default_quality, format_opts, list_qualities
...
def test_only_offers_heights_that_exist():
    ...
    assert keys(list_qualities(info)) == ["best", "h720"]
...
def test_portrait_video_uses_short_side():
    ...
    assert keys(list_qualities(info)) == ["best", "h1080"]


def test_audio_only_source_has_no_video_qualities():
    ...
    assert keys(list_qualities(info)) == []
...
def test_single_url_without_formats_list():
    ...
    assert keys(list_qualities(info)) == ["best", "h480"]
...
def test_default_quality_uses_setting_when_available():
    ...
    assert default_quality(qs, "audio").key == "best"  # audio is a format now, not a quality
...
def test_format_opts():
    v = format_opts(Quality("h1080", "1080p", height=1080), FORMATS_BY_KEY["original"])
    assert v["format_sort"][0] == "res:1080"
    assert "vcodec:h264" in v["format_sort"]
    assert v["merge_output_format"] == "mp4"
    assert format_opts(None, FORMATS_BY_KEY["edit"])["format_sort"][0] == "res"
    a = format_opts(None, FORMATS_BY_KEY["mp3"])
    assert a["postprocessors"][0]["key"] == "FFmpegExtractAudio"
    assert a["postprocessors"][0]["preferredcodec"] == "mp3"
    assert format_opts(None, FORMATS_BY_KEY["wav"])["postprocessors"][0]["preferredcodec"] == "wav"
    j = format_opts(None, FORMATS_BY_KEY["jpg"])
    assert j["skip_download"] and j["writethumbnail"]
    assert [f.ext for f in FORMATS_BY_KEY.values()] == ["mp4", "mp4", "mov", "mp3", "wav", "jpg"]
